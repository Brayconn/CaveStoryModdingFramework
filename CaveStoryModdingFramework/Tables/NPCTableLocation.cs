...
{
    public static class NPCTableExtensions
    {
    ...
    public class NPCTableLocation : DataLocation
    {
        ...
        public NPCTableLocation(string filename)
        {
            Filename = filename;
            DataLocationType = DataLocationTypes.External;
        }

        public List<NPCTableEntry> Read()
        {
            if (!TryCalculateEntryCount(NPCTableEntry.Size, out int npcCount))
                npcCount = NpcCount;
            using (var br = new BinaryReader(GetStream(FileMode.Open, FileAccess.Read)))
            {
                switch(NpcTableFormat)
                {
                    ...
                    default:
                        throw new ArgumentException("Invalid NPC Table format!", nameof(NpcTableFormat));
                }
            }
        }

        public void Write(IList<NPCTableEntry> table)
        {
        ...
        {
            if (obj is NPCTableLocation l)
            {
                return base.Equals(l)
                    && NpcTableFormat == l.NpcTableFormat
                    && NpcCount == l.NpcCount;
            }
            else
